...
} from 'redux/errorHandlingTypes/rejectErrorTypes';
...
>('auth/refresh', async (_, thunkAPI) => {
  ...
  const savedToken = (state as RootState).auth.token;

  if (savedToken === null) {
    throw thunkAPI.rejectWithValue({
      message: 'Can`t fetch with current user ^_^',
    });
  }
